  ...
  <UContainer>
    ...
    <div v-else>
      <div v-if="secondHand" class="my-8">
        <AppStoreTopSection
          linkLabel="Wróć do listy sklepów"
          backLink="/city/{{ secondHand.city_id }}"
        >
          <template #title>
            {{ secondHand?.name }}
          </template>
        ...
        <ClientOnly>
          ...
          >
            ...
            <LMarker
              :lat-lng="[secondHand.lat, secondHand.lng]"
              :icon="selectedSecondHandIcon"
            >
              <LPopup>
            ...
            <LMarker
              v-for="restSecondHand in restSecondHands"
              :lat-lng="[restSecondHand.lat, restSecondHand.lng]"
              :icon="defaultIcon"
            >
              <LPopup>
        ...
        <div
          v-if="secondHand.schedule[todayKey].delivery"
        ...
        <span
          v-if="
            secondHand.schedule[todayKey] && secondHand.schedule[todayKey].promo
          "
...
onMounted(() => {
  import("leaflet").then((L) => {
    import("~/assets/img/secondhand_marker.svg").then((secondhandSvg) => {
      import("~/assets/img/secondhand_marker_selected.svg").then(
        (secondhandSvgSelected) => {
...
async function getSecondHand() {
  try {
    ...
    secondHand.value = data.find(
      (secondHand: SecondHand) => Number(secondHand.id) === Number(id)
    );
    restSecondHands.value = data.filter(
      (secondHand: SecondHand) => Number(secondHand.id) !== Number(id)
    );
