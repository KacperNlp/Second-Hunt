                ...
                <UCard class="p-6">
                    <form @submit.prevent="handleSubmit" class="space-y-4">
                        ...
                        <div class="flex bg-gray-100 rounded-lg p-1 mb-6">
                            <button type="button" @click="switchToUser" :class="[
                            ...
                            <button type="button" @click="switchToOwner" :class="[
                                'flex-1 py-2 px-4 rounded-md text-sm font-medium transition-colors cursor-pointer',
                                isOwner
                                    ? 'bg-white text-primary shadow-sm'
                                    : 'text-gray-600 hover:text-gray-800'
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <UInput v-model="form.lastName" placeholder="Kowalski" :error="errors.lastName"
                                    required />
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <UInput v-model="form.email" type="email" placeholder="jan.kowalski@example.com"
                        ...
                        <div v-if="isOwner" class="space-y-4 border-t pt-4">
                            ...
                            <div>
                                ...
                                <UInput v-model="form.storeName" placeholder="Second Hand Kowalski"
                            ...
                            <div>
                                ...
                                <UInput v-model="form.storeAddress" placeholder="ul. Przykładowa 1, 00-001 Warszawa"
                                    :error="errors.storeAddress" required />
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <USelect v-model="form.province" :items="provinces" option-attribute="label"
                                        value-attribute="value" placeholder="Wybierz województwo"
                                        :error="errors.province" required />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">
                                        Miasto *
                            ...
                            <div>
                                ...
                                <UInput v-model="form.zipCode" placeholder="00-001" :error="errors.zipCode"
                                    @input="formatZipCode" required />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <UInput v-model="form.password" type="password" placeholder="Minimum 8 znaków"
                        ...
                        <div class="flex items-start gap-2">
                            ...
                            <label class="text-sm text-gray-600">
                                Akceptuję
                                <a href="/terms" class="text-primary hover:underline">regulamin</a>
                                i
                                <a href="/privacy" class="text-primary hover:underline">politykę prywatności</a> *
                            </label>
                        </div>

                        <!-- Newsletter (User only) -->
                        <div v-if="!isOwner" class="flex items-start gap-2">
                            <UCheckbox v-model="form.newsletter" />
                            <label class="text-sm text-gray-600">
                                Chcę otrzymywać informacje o promocjach i nowych sklepach
                            </label>
                        </div>

                        <!-- Submit Button -->
                        <UButton type="submit" color="primary" size="lg" class="w-full" :loading="isSubmitting"
...
const userType = route.query.type as string
...
// Set default user type if none provided
onMounted(() => {
    if (!userType) {
        router.push({ query: { type: 'user' } })
    }
