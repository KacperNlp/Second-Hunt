<template>
  <UContainer>
    ...
    <div v-else>
      <AppStoreTopSection linkLabel="Wróć do strony głównej" backLink="/">
        <template #title>
          {{ selectedCity?.name }}
        </template>
      </AppStoreTopSection>
      <section
        v-if="secondHands.length > 0"
        class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8"
      >
        <NuxtLink
          class="relative"
          v-for="secondHand in sortedSecondHands"
          :key="secondHand.name"
          :to="`/secondhand/${secondHand.id}`"
        >
          <UCard
            variant="solid"
            class="h-full border-2 border-white hover:border-[#D4A259] duration-300"
          >
            ...
            <div class="flex flex-col gap-2">
              ...
              <span
                v-if="secondHand.schedule[todayKey].promo"
